#!/usr/bin/env python3
"""
ProtonMail OAuth2 Helper for Authentication
This script helps you get OAuth2 tokens for ProtonMail API access.
"""
...
class ProtonMailOAuth2Helper:
    """Helper class for ProtonMail OAuth2 authentication"""
    
    def __init__(self, client_id: str, client_secret: str):
        self.client_id = client_id
        self.client_secret = client_secret
        self.redirect_uri = "http://localhost:8080"
        self.scope = "mail:read mail:write"
        
    def get_authorization_url(self) -> str:
        """Generate the authorization URL for ProtonMail OAuth2"""
        params = {
            'client_id': self.client_id,
            'response_type': 'code',
            'redirect_uri': self.redirect_uri,
            'scope': self.scope,
            'state': 'protonmail_oauth2'
        }
        
        query_string = urllib.parse.urlencode(params)
        return f"https://account.proton.me/oauth/authorize?{query_string}"
...
class OAuth2CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle the OAuth2 callback from ProtonMail"""
        if self.path.startswith('/?'):
            ...
            if 'code' in params:
                ...
                try:
                    ...
                    # Save tokens to file
                    with open('protonmail_oauth2_tokens.json', 'w') as f:
                        json.dump(tokens, f, indent=2)
                    print("\n💾 Tokens saved to 'protonmail_oauth2_tokens.json'")
    ...
    def send_success_response(self):
        ...
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            <h1>✅ ProtonMail OAuth2 Authentication Successful!</h1>
            <p>You can now close this window and return to the terminal.</p>
            <p>Your tokens have been saved to 'protonmail_oauth2_tokens.json'</p>
        </body>
        </html>
        """
...
def main():
    """Main function to run the ProtonMail OAuth2 helper"""
    print("🔐 ProtonMail OAuth2 Token Helper")
    print("=" * 50)
    ...
    oauth2_helper = ProtonMailOAuth2Helper(client_id, client_secret)
    ...
    print("3. Sign in with your ProtonMail account")
