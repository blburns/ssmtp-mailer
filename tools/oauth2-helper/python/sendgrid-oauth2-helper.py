#!/usr/bin/env python3
"""
SendGrid OAuth2 Helper for API Authentication
This script helps you get OAuth2 tokens for SendGrid API access.
"""
...
class SendGridOAuth2Helper:
    """Helper class for SendGrid OAuth2 authentication"""
    ...
    def get_authorization_url(self) -> str:
        """Generate the authorization URL for SendGrid OAuth2"""
        params = {
            'client_id': self.client_id,
            'response_type': 'code',
            'redirect_uri': self.redirect_uri,
            'scope': self.scope,
            'state': 'sendgrid_oauth2'
        }
        
        query_string = urllib.parse.urlencode(params)
        return f"https://api.sendgrid.com/v3/oauth/authorize?{query_string}"
...
class OAuth2CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle the OAuth2 callback from SendGrid"""
        if self.path.startswith('/?'):
            ...
            if 'code' in params:
                ...
                try:
                    ...
                    # Save tokens to file
                    with open('sendgrid_oauth2_tokens.json', 'w') as f:
                        json.dump(tokens, f, indent=2)
                    print("\n💾 Tokens saved to 'sendgrid_oauth2_tokens.json'")
    ...
    def send_success_response(self):
        ...
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            <h1>✅ SendGrid OAuth2 Authentication Successful!</h1>
            <p>You can now close this window and return to the terminal.</p>
            <p>Your tokens have been saved to 'sendgrid_oauth2_tokens.json'</p>
        </body>
        </html>
        """
...
def main():
    """Main function to run the SendGrid OAuth2 helper"""
    print("🔐 SendGrid OAuth2 Token Helper")
    print("=" * 50)
    ...
    oauth2_helper = SendGridOAuth2Helper(client_id, client_secret)
    ...
    print("3. Sign in with your SendGrid account")
