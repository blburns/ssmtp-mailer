#!/usr/bin/env python3
"""
Zoho Mail OAuth2 Helper for Authentication
This script helps you get OAuth2 tokens for Zoho Mail API access.
"""
...
class ZohoMailOAuth2Helper:
    """Helper class for Zoho Mail OAuth2 authentication"""
    
    def __init__(self, client_id: str, client_secret: str):
        self.client_id = client_id
        self.client_secret = client_secret
        self.redirect_uri = "http://localhost:8080"
        self.scope = "ZohoMail.messages.READ,ZohoMail.messages.CREATE"
        
    def get_authorization_url(self) -> str:
        """Generate the authorization URL for Zoho Mail OAuth2"""
        params = {
            'client_id': self.client_id,
            'response_type': 'code',
            'redirect_uri': self.redirect_uri,
            'scope': self.scope,
            'state': 'zoho_mail_oauth2'
        }
        
        query_string = urllib.parse.urlencode(params)
        return f"https://accounts.zoho.com/oauth/v2/auth?{query_string}"
...
class OAuth2CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle the OAuth2 callback from Zoho Mail"""
        if self.path.startswith('/?'):
            ...
            if 'code' in params:
                ...
                try:
                    ...
                    # Save tokens to file
                    with open('zoho_mail_oauth2_tokens.json', 'w') as f:
                        json.dump(tokens, f, indent=2)
                    print("\n💾 Tokens saved to 'zoho_mail_oauth2_tokens.json'")
    ...
    def send_success_response(self):
        ...
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            <h1>✅ Zoho Mail OAuth2 Authentication Successful!</h1>
            <p>You can now close this window and return to the terminal.</p>
            <p>Your tokens have been saved to 'zoho_mail_oauth2_tokens.json'</p>
        </body>
        </html>
        """
...
def main():
    """Main function to run the Zoho Mail OAuth2 helper"""
    print("🔐 Zoho Mail OAuth2 Token Helper")
    print("=" * 50)
    ...
    oauth2_helper = ZohoMailOAuth2Helper(client_id, client_secret)
    ...
    print("3. Sign in with your Zoho Mail account")
