...
# Function to detect Red Hat distribution
detect_distribution() {
    ...
    print_status "Detected distribution: $DISTRO_NAME $DISTRO_VERSION ($DISTRO_ID)"
}

# Function to check and install dependencies
check_dependencies() {
    print_status "Checking and installing Red Hat/CentOS/Fedora dependencies..."
    ...
    sudo $PKG_MANAGER install -y "${build_packages[@]}"
    
    print_status "Installing SSL libraries..."
    sudo $PKG_MANAGER install -y "${ssl_packages[@]}"
    
    print_status "Installing JSON and HTTP libraries..."
    sudo $PKG_MANAGER install -y "${lib_packages[@]}"
    ...
    if [ "$CREATE_PACKAGES" = true ]; then
        ...
        if command -v cpack &> /dev/null; then
            if cpack -G RPM; then
                ...
                if [ -d "$DIST_DIR" ]; then
                    print_status "Moving RPM package to dist directory..."
                    mv *.rpm "$DIST_DIR/" 2>/dev/null || true
                    print_success "RPM package moved to dist directory"
                fi
            else
...
show_build_summary() {
    ...
    if [ "$CREATE_PACKAGES" = true ]; then
        echo "  - Install RPM package: sudo rpm -i $DIST_DIR/*.rpm"
    fi
}
