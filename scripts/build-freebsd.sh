...
# ssmtp-mailer FreeBSD Build Script
# This script automates the build process for FreeBSD systems
...
OPTIONS:
    ...
    --package               Build FreeBSD package after successful build
...
EXAMPLES:
    ...
    $0 --package             # Build and create FreeBSD package
...
# Function to detect FreeBSD version
detect_freebsd() {
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        FREEBSD_VERSION="$VERSION_ID"
        FREEBSD_NAME="$NAME"
    elif [ -f /etc/version ]; then
        FREEBSD_VERSION=$(cat /etc/version | awk '{print $2}')
    ...
    print_status "Detected FreeBSD version: $FREEBSD_NAME $FREEBSD_VERSION"
}

# Function to check and install dependencies
check_dependencies() {
    print_status "Checking and installing FreeBSD dependencies..."
...
# Function to create FreeBSD package
create_freebsd_package() {
    if [ "$CREATE_PACKAGES" = true ]; then
        print_status "Creating FreeBSD package..."
        cd "$BUILD_DIR"
        
        # Create FreeBSD package using CPack
        if command -v cpack &> /dev/null; then
            if cpack -G TGZ; then
                print_success "FreeBSD package (TGZ) created"
                
                # Move package to dist directory
                if [ -d "$DIST_DIR" ]; then
                    print_status "Moving FreeBSD package to dist directory..."
                    mv *.tar.gz "$DIST_DIR/" 2>/dev/null || true
                    print_success "FreeBSD package moved to dist directory"
                fi
            else
                print_warning "FreeBSD package creation failed"
            fi
        else
            print_warning "cpack not found, skipping FreeBSD package creation"
        fi
    fi
}
...
show_build_summary() {
    ...
    echo
    echo "Build Summary:"
    echo "  FreeBSD Version: $FREEBSD_NAME $FREEBSD_VERSION"
    ...
    if [ "$CREATE_PACKAGES" = true ]; then
        echo "  - Install FreeBSD package: sudo pkg add $DIST_DIR/*.tar.gz"
    fi
}
...
main() {
    ...
    # Create FreeBSD package if requested
    create_freebsd_package
